...
await copyFile(resolve(PUBLIC, 'manifest.json'), resolve(DIST, 'manifest.json'))
...
async function generatePngIcon(size) {
  ...
  const bgR = 10, bgG = 10, bgB = 10      // #0a0a0a 深色背景
  const fgR = 0, fgG = 255, fgB = 0        // #00ff00 终端绿
  const borderR = 0, borderG = 180, borderB = 0  // 边框深绿
  ...
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      ...
      const inInnerRect = isInRoundedRect(x, y, size - borderW * 2, size - borderW * 2, radius - borderW)

      if (!inRect) {
        pixels[idx] = pixels[idx + 1] = pixels[idx + 2] = pixels[idx + 3] = 0
      } else if (!inInnerRect) {
        pixels[idx] = borderR; pixels[idx + 1] = borderG; pixels[idx + 2] = borderB; pixels[idx + 3] = 255
      } else {
...
function drawT(px, py, cx, cy, size) {
  ...
  if (py >= topY && py <= topY + thickness &&
      px >= cx - width / 2 && px <= cx + width / 2) {
    return true
  }
  if (py >= topY && py <= topY + height &&
      px >= cx - thickness / 2 && px <= cx + thickness / 2) {
    return true
  }
  return false
}
